...
def show_project_detail():
    ...
    try:
        ...
        if reference_ksps_current:

            current_reference = (
                reference_ksps_current[-1]
            )

            st.info(
                f"Aktuálne používaný referenčný KSP: "
                f"**{current_reference['file_name']}**"
            )

        else:

            st.warning(
                "Projekt zatiaľ nemá referenčný KSP."
            )

        # ==================================================
        # NAHRADENIE REFERENČNÉHO KSP
        # ==================================================

        with st.expander(
            "🔄 Nahradiť referenčný KSP"
        ):

            replacement_reference = (
                st.file_uploader(
                    "Vyber nový referenčný KSP",
                    type=[
                        "xlsx",
                        "xls"
                    ],
                    key=(
                        f"replace_reference_"
                        f"{project_id}"
                    )
                )
            )
            ...
            if st.button(
                "Nahradiť aktuálny referenčný KSP",
                use_container_width=True,
                key=f"replace_reference_btn_{project_id}"
            ):

                if not replacement_reference:

                    st.warning(
                        "Najprv vyber nový referenčný KSP."
                    )
                ...
                try:
                    ...
                    # Najprv uložíme nový referenčný KSP
                    upload_project_file(
                        project_id,
                        replacement_reference,
                        "reference_ksp"
